...
const createFormState = ({price, email, confirmationEmail}, hasAgreedTOS, isNewPurchase) => {
    ...
    if (!hasAgreedTOS) {
        return _createFormState(true, "You need to agree with the terms of service in order to submit")
    }

    return _createFormState()
}

export default function OrderModal({nfts, onClose, onSubmit, isNewPurchase}) {
        ...
        <Modal isOpen={isOpen}>
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div className="sm:flex sm:items-start">
                    ...
                    <h3 className="mb-4 text-lg leading-6 text-gray-900" id="modal-title">
                        <span className="text-2xl font-semibold">{nfts.title}</span>
                        <br></br>
                        <span className="text-xl ">Please provide a <span className="font-semibold">valid</span> email to receive special perks that come with each <span className="font-semibold">NFT</span></span>
                    ...
                    <div className="text-xs text-gray-700 flex mt-4">
                        <label className="flex items-center mr-2">
                        <input
                        ...
                        <span>I accept UpDownStudios &apos;terms of service&apos; and I agree that my order can be rejected in the case data provided above are not correct</span>
                    </div>
                    { formState.message &&
                        <div className="p-4 my-3 text-red-700 bg-red-200 
                        rounded-lg text-sm">
                            { formState.message }
                        </div>
                    }
